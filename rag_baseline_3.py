...
def ingest_multiple_webpages_from_file(filepath="webpage_urls.txt"):
...
def main():
    ...
    if choice == "1":
        ...
        if mode == "web":
            ...
            filepath = input(
                "Path to URL list file (default: webpage_urls.txt): ").strip()
            if not filepath:
                filepath = "webpage_urls.txt"
            ingest_multiple_webpages_from_file(filepath)
        elif mode == "yt":
